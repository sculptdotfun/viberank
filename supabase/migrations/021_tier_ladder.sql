-- Migration 021: recalibrated tier ladder.
--
-- get_site_stats() buckets the /stats tier counts in SQL, so the thresholds
-- live here as well as in src/lib/tiers.ts (test/tiers.test.mts keeps them in
-- step). Lifetime totals only grow: by September 2026 the $50K Supernova held
-- the top 6% of developers rather than the 0.5% it was calibrated for. The
-- lower rungs keep their thresholds; Inferno moves to $25K, Supernova to
-- $100K, and Hypernova ($250K) is the new top tier. Everything else in the
-- function is unchanged from 008.
...
AS $$
  SELECT jsonb_build_object(
    ...
    'tiers', COALESCE((
      ...
      FROM (
        SELECT CASE
                 WHEN best >= 250000 THEN 'hypernova'
                 WHEN best >= 100000 THEN 'supernova'
                 WHEN best >= 25000 THEN 'inferno'
