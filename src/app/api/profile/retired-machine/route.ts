...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json(await dataLayer.submissions.previewRetiredMachine(username, from, to));
}
...
export async function POST(request: NextRequest) {
  ...
  if (!validRange(from, to)) return invalid();
  return NextResponse.json(await dataLayer.submissions.retireUnattributed(username, from, to as string));
}

export async function DELETE(request: NextRequest) {
  const { dataLayer, username } = await owner(request);
  if (!username) return unauthorized();
  return NextResponse.json(await dataLayer.submissions.restoreUnattributed(username));
}
